...
const ValidationAirtm = ({ route }) => {
  ...
  useEffect(() => {
    const loadTime = async () => {
      const jwt_time = await AsyncStorage.getItem("timeairtm");
  ...
  const handleAnnuler = async () => {
    try {
      const response = await Axios.post(`${BASE_URL}/airtm/cancelbyuser`, {
        idairtmtransaction: id,
      });
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.viewInterieur}>
          <Text style={styles.texte1}>Retrait Airtm</Text>
...
export default ValidationAirtm;
