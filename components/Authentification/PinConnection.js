...
const PinConnection = ( {route} ) => {
  ...
  const handleFingerprintAuthentication = async () => {
    try {
      ...
      if (success) {
        const response = await Axios.post(`${BASE_URL}/users/loginempreinte`, {email: email});
  ...
  const handleFaceIdAuthentication = async () => {
    try {
      ...
      console.log("Erreur d'authentification par reconnaissance faciale :", error);
    }
  };
  ...
  const handlePinValidation = async () => {
    const response = await Axios.post(`${BASE_URL}/users/login`, {email: email, pin: enteredPin});
