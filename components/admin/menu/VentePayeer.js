...
const VentePayeer = () => {
  ...
  const [ventePayeerHistory, setVentePayeerHistory] = useState([]);

  const getVentePayeerHistory = async() => {
    
      try{

        const apiUrl = `${BASE_URL}/transactionhistory/filtered`;

        const response = await Axios.post(apiUrl, { type: 'depot', portefeuille: 'payeer' });

        const venteUsdt = response.data;

        setVentePayeerHistory(venteUsdt);
  ...
  return (
    <ScrollView>
      <View>
        ...
        {ventePayeerHistory.map((item) => (
          <View key={item.id} style={styles.row}>
            <Text style={styles.cell}>{item.numeroordre}</Text>
            <Text style={styles.cell}>{item.montant} USD</Text>
            <Text style={styles.cell}>{formatNumber(item.cours)} Ar</Text>
            <Text style={styles.cell}>{formatNumber(item.montantmga)} Ar</Text>
