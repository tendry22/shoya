...
const AchatUsdt = () => {
  ...
  const [achatUsdtHistory, setAchatUsdtHistory] = useState([]);

  const getAchatUsdtHistory = async() => {
      try{

        const apiUrl = `${BASE_URL}/transactionhistory/filtered`;

        const response = await Axios.post(apiUrl, { type: 'retrait', portefeuille: 'usdt' });

        const achatUsdt = response.data;

        setAchatUsdtHistory(achatUsdt);
