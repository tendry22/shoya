...
const AchatPayeer = () => {
  ...
  const [achatPayeerHistory, setAchatPayeerHistory] = useState([]);

  const getAchatPayeerHistory = async() => {
    
      try{

        const apiUrl = `${BASE_URL}/transactionhistory/filtered`;

        const response = await Axios.post(apiUrl, { type: 'retrait', portefeuille: 'payeer' });

        const achatUsdt = response.data;

        setAchatPayeerHistory(achatUsdt);
