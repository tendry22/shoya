...
const VentePm = () => {
  ...
  const [ventePmHistory, setVentePmHistory] = useState([]);

  const getVentePmHistory = async() => {
    
      try{

        const apiUrl = `${BASE_URL}/transactionhistory/filtered`;

        const response = await Axios.post(apiUrl, { type: 'depot', portefeuille: 'pm' });

        const venteUsdt = response.data;

        setVentePmHistory(venteUsdt);
