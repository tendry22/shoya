...
const AchatAirtm = () => {
  ...
  const [achatAirtmHistory, setAchatAirtmHistory] = useState([]);

  const getAchatAirtmHistory = async() => {
    
      try{

        const apiUrl = `${BASE_URL}/transactionhistory/filtered`;

        const response = await Axios.post(apiUrl, { type: 'retrait', portefeuille: 'airtm' });

        const achatUsdt = response.data;

        console.log(achatUsdt);

        setAchatAirtmHistory(achatUsdt);
