...
const AdminAirtm = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        //await sendPushNotification(token, 'Airtm :Aucun Transaction', 'Personne n`a fait de transaction Airtm');
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAirtm = async () => {
      try {
        const response = await Axios.get(`${BASE_URL}/airtm/pending`);
        ...
        if (!areArraysEqual(newData, listeAirtm)) {
          ...
          if (adminExpoToken) {
            ...
            await schedulePushNotification({
              adminExpoToken,
              title: "Une transaction en attente Airtm",
              body: `Airtm : Une transaction est en attente de validation.`,
  ...
  return (
    ...
    >
      ...
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Validation Airtm</Text>
      </View>
